        ...
        new SQLBatch(){
            ...
            protected void scripts() {
                long count = sql("select count(vm.uuid)" +
                        ...
                        " and vm.state not in :vmState")
                        .param("vmUuid",msg.getVmInstanceUuid())
                        .param("vmState", VmInstanceState.Stopped)
                        .param("platformType", ImagePlatform.Other).find();
                if(count > 0){
                   throw new ApiMessageInterceptionException(operr("the vm[uuid:%s] doesn't suport to hot plugin attach volume[%s] on the basis of that the image platform type of the vm is other ", msg.getVmInstanceUuid(), msg.getVolumeUuid()));
                }
