...
public class NetworkUtils {
    ...
    public static boolean isIpv4GroupAddress(String ip) {
        if (isInRange(ip, "224.0.0.0", "240.255.255.255")) {
            return true;
        }
        return false;
    }

    public static boolean isIpv4LocalAddress(String ip) {
    ...
    public static boolean isIpv4LinkLocalAddress(String ip) {
    ...
    public static boolean isUnicastIPAddress(String ip) {
        if (isIpv4Address(ip)) {
            if (isIpv4GroupAddress(ip) || isIpv4LocalAddress(ip) || isIpv4LinkLocalAddress(ip)) {
                return false;
            } else {
                return true;
            }
    ...
    public static boolean isMulticastIPAddress(String ip) {
        if (isIpv4Address(ip)) {
            if (isIpv4GroupAddress(ip)) {
                return true;
            } else {
                return false;
            }
        } else {
            return isIpv6MulticastAddress(ip);
        }
    }
    ...
    public static boolean isUnicastCidr(String cidr) {
        try {
            IPv6Network network6 = IPv6Network.fromString(cidr);
            String formatCidr = network6.toString();
            String formatAddress = formatCidr.split("/")[0];
            IPv6Address address6 = IPv6Address.fromString(formatAddress);
            if (address6.isMulticast() || address6.isSiteLocal() || address6.isLinkLocal()) {
                return false;
            } else {
                return true;
            }
            ...
            SubnetUtils sub = new SubnetUtils(cidr);
            String address = sub.getInfo().getAddress();
            if (isIpv4GroupAddress(address) || isIpv4LocalAddress(address) || isIpv4LinkLocalAddress(address)) {
                return false;
            } else {
                return true;
            }
